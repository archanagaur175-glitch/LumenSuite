...
1. **Platform**: currently `windows-latest` only. (The original 3-platform matrix was scaled back
   to focus on Windows first; Linux/macOS packaging is intentionally disabled in the workflow.)
2. **LibreOffice**: downloads the pinned version from `download.documentfoundation.org`
   (`scripts/fetch-libreoffice.mjs`), extracts it via `msiexec /a` administrative install,
   computes and logs SHA-256 + size, then slims the runtime (`scripts/package-libreoffice.mjs` —
   strips help/examples/templates/gallery/python).
3. **Engine gate**: `scripts/smoke-convert.mjs` runs against the actual extracted runtime:
   ...
   The job fails if any conversion does not produce a non-empty output file. Note: `--convert-to`
   must be passed as a bare `--convert-to` arg followed by the filter string as a separate argv
   element (filter names contain spaces, e.g. `docx:MS Word 2007 XML`).
...
6. **Upload**: a `release` job downloads each platform artifact and publishes a GitHub Release
   with `softprops/action-gh-release` (files: installers `.msi`/`.exe`).
...
2. **Linux / macOS**: packaging is disabled in the workflow while Windows is the focus. When
   re-enabled: macOS signing + notarization requires `APPLE_CERTIFICATE`,
   `APPLE_CERTIFICATE_PASSWORD`, `APPLE_SIGNING_IDENTITY`, `APPLE_ID`, `APPLE_PASSWORD`,
   `APPLE_TEAM_ID` (paid Apple Developer account), else macOS ships unsigned.
